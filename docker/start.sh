...
/bin/echo "0 30 * * * /usr/local/bin/python /usr/src/app/flowyt/cli.py -w sapi -f vulnerabilities -d true >> /dev/stdout 2>&1" >>/tmp/jobs.cron
/bin/echo "0 1 * * * /usr/local/bin/python /usr/src/app/flowyt/cli.py -w massification -f chain -d true >> /dev/stdout 2>&1" >>/tmp/jobs.cron
